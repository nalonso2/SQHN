...
############################################################################################################################
def train_online(in_dim, chnls, in_chn, data, dev='cuda', max_iter=200, num_seeds=10, beta=10, lr=.2, shuf=True,
                 num_up=1, ns_type=0, optim=0, arch=2):
    with torch.no_grad():
    ...
    with open(f'data/AATree_NoisyOnlineBP_data{data}_opt{optim}_NUp{num_up}_NsType{ns_type}.data', 'wb') as filehandle:
