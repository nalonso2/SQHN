...
def train_online(in_sz, hid_sz, simf, data, dev='cuda', max_iter=200, wtupType=1, num_seeds=10, alpha=10, lr=.2,
                 t_fq=100, shuf=True, det_type=0, ns_type=0, num_up=5, plus=False):
    with torch.no_grad():
        ...
        for s in range(num_seeds):
            model = Unit.MemUnit(layer_szs=[in_sz, hid_sz], simFunc=simf, wt_up=wtupType, alpha=alpha, lr=lr, det_type=det_type).to(dev)
            ...
            for batch_idx, (images, y) in enumerate(train_loader):
                ...
                for n in range(num_up):
                    ...
                    #If first iteration or not doing IPHN+ then update activities
                    if n ==0 or not plus:
        ...
        print(f'Plus:{plus}   Wt Update {wtupType}  DetType:{det_type}  PropRcl:{torch.mean(recall_pcnt[:, -1])} '
              f'MSE:{torch.mean(recall_mse[:, -1])} '
              f'MSE(Noise):{torch.mean(recall_mse_n[:, -1])}')

    with open(f'data/AA_NoisyOnline_numN{hid_sz}_data{data}_numData{max_iter}_NUp{num_up}_NsType{ns_type}_plus{plus}.data', 'wb') as filehandle:
